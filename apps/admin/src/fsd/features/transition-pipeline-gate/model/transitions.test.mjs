...
describe("GATE_LOCK_LABEL·rejectLockLabel — 잠금 칩 문구(리터럴 고정)", () => {
  it("도장 잠금 문구는 '도장 찍음'으로 고정", () => {
    assert.equal(GATE_LOCK_LABEL, "도장 찍음");
  });

  it("반려 잠금 문구는 액션별 낱말(3분기)", () => {
    assert.equal(rejectLockLabel("bounce"), "되돌림");
    assert.equal(rejectLockLabel("hold"), "보류함");
    assert.equal(rejectLockLabel("discard"), "폐기함");
  });
});
