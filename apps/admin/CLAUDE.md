...
| `/pipeline/agents/[agent]` | 행위자 상세(읽기 전용). 역할은 `.claude/agents/<id>.md` frontmatter + 본문, 기록은 뷰어 링크. roster 닫힌 목록 밖은 `notFound` |
...
현재 **27개 파일, 58개 suite, 276개 test**다. 아래 첫 열은 `src/**/*.test.mjs` 전체 집합이며 파일마다 정확히 한 번만 적는다.
...
| `src/fsd/entities/repo-doc/api/queries.test.mjs` | raw 문서 GET(화이트리스트 밖→fetch 없이 null, 404→null, 비-OK→throw), roster 정의 파일 통과·비-roster 정의 거부, plans 목록 → 항목 ID 집합 |
...
| `src/fsd/shared` | generic UI, class utility, result type, observability wrapper, roster 멤버십 상수(`agents/roster.ts` — entities와 pages가 함께 읽는 유일한 하위 계층) |
...
- 저장소 문서 GET owner는 `src/fsd/entities/repo-doc/api/queries.ts`다(FEAT-14, 읽기 전용). 문서 본문은 raw CDN, `docs/plans/` 목록은 contents API다. **URL 파라미터가 fetch 경로가 되므로** 이 owner는 fetch 직전에 `isWhitelistedDocPath`로 경로를 다시 검사한다 — 라우트의 slug 검증에 이어지는 두 번째 방어선이다. FEAT-15부터 같은 owner가 `isAgentDefinitionPath`(`src/fsd/shared/agents/roster.ts`)도 통과시킨다 — 접두사 규칙이 아니라 roster 닫힌 목록에서 조립한 `.claude/agents/<id>.md` 정확 경로 집합이다.
