...
- [ ] FEAT-09: `/pipeline` 결재함에 반려 경로 — 게이트 거절을 대시보드에서
  ...
  area: apps/admin
  status: 검토대기
  근거: 사용자 직접 발주 — pm 경유 없음(소유자 발주로 기록). FEAT-08이 승인(도장)만 만들고 거절 수단을 남기지 않아, 계획 반려·항목 보류·폐기가 전부 대시보드 밖에서만 가능하다(FEAT-01을 내리는 데 실제로 수동 처리가 필요했다). 발주 계약은 TASK_BACKLOG.md의 FEAT-09 항목이 원천 — 거절 세 갈래(되돌리기·보류·폐기)의 구분, FEAT-08 화이트리스트 재사용, 사유 기록, 폐기의 비가역성 취급을 계획 단계에서 판단해야 한다. FEAT-01은 같은 날 보류로 내려 미결은 이 항목 1건이다. **게이트①은 대시보드 도장 버튼으로 열 예정 — FEAT-08 원격 쓰기 경로의 첫 실측을 겸한다.**
  결과: 계획서를 `docs/plans/FEAT-09.md`에 작성했다. `transitions.ts`의 `GATE_TRANSITIONS` 화이트리스트를 재사용해 반려 셋(되돌리기: 검토대기→계획지시, 보류: 승인대기·검토대기→보류+결과 필드 기록, 폐기: 항목 블록 통째 제거)을 순수 함수로 추가하고, `commit-transition.ts`와 같은 GET→편집→PUT 왕복을 신규 `reject-transition.ts`가 공유 헬퍼로 묶는다. UI는 도장과 같은 임프린트 문법에 `--hold` 잉크만 바꾼 반려 트리거 + 팝오버 3단 확인(폐기만 ID 타이핑 확인)으로 frontend-design 2-pass를 거쳐 설계했다(계획서 "디자인 방향" 절). `TASK_BACKLOG.md` 편집(폐기 시 백로그 정리)은 백로그의 out-of-scope 명시에 따라 이번 계획에서 제외했다 — UI 문구로만 안내한다. 코드는 고치지 않았다(계획 단계).
