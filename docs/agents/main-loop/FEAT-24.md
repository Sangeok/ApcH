...
## 루틴 지침 교체 (2026-08-27, 메인 루프가 실행)

계획서는 이를 「범위 밖 의존」(사용자 몫)으로 적었고 메인 루프도 그렇게 보고했으나, 사용자 지적으로
재확인한 결과 **메인 루프가 `RemoteTrigger`(`/schedule` 스킬)로 직접 교체할 수 있었다.** "범위 밖"은
admin-dev의 저장소 쓰기 범위 기준이었지 메인 루프의 능력 기준이 아니었다 — 잘못된 안내였고 정정한다.

- 대상: 루틴 `pipeline-command`(`trig_013BviNQtefJEXiXyPhkHCff`), `action: update`로 프롬프트만 교체.
  환경·모델(sonnet)·도구셋·저장소·MCP 연결은 기존 값 그대로 재전송해 보존. `updated_at` 2026-08-27T05:04:26Z.
- 계획서 산출 문안을 그대로 쓰되 **한 줄을 보존**했다: 기존 지침의 "예: FEAT-09가 계획지시면
  admin-dev.md의 A단계 규칙을 읽고 네가 직접 docs/plans/FEAT-09.md를 쓴다." 이 줄은 진행 코멘트 규약과
  무관한 기존 지시라, 계획서 문안이 그것을 누락한 것은 의도된 삭제가 아니라 재작성 과정의 누락으로 봤다
  (지우면 FEAT-24와 무관한 퇴행이 된다). 그 외는 계획서 문안 그대로.
- 저장소 계약 사본 3곳 갱신(`remote-agent-pipeline-generalization.md`): 「루틴 지침 템플릿」 본문 +
  진행 코멘트 규약 문단, 「읽기 ②」 전제 (a)(b)(접두 셋·종료 답글 1건), 「알려진 약점」 연속 클릭 행
  (FEAT-24가 UI에서 재클릭을 막는다). `post-pipeline-command.ts` 주석 사본은 구현에 이미 포함됐다.

**다음 원격 실행부터 진행 코멘트가 쌓인다** — 배포 후 `running` pill 관측의 선행 조건이 이로써 충족.
