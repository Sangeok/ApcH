...
## 라운드 5 (plan-verifier 독립 패스 4사이클) — 클린

**결함 0건.** 계획서에서 라운드 기록 111줄을 걷어내 감사 표면이 구현 지시로만 좁혀진 직후의
사이클이고, 바로 통과했다. 앞선 세 사이클의 소득이 전부 그 산문 표면에서 나왔다는 진단이 맞았다.

**이번 사이클이 가장 멀리 갔다** — 앞선 셋보다 실행 범위가 넓다.

- **게이트 4종을 실제로 돌렸다**: `npm test` **77/0**, `tsc --noEmit` clean, `next lint`
  경고·에러 0, **`npm run build` 성공**. 그리고 신규 `.mjs`가 게이트를 깨지 않는 이유를
  스코프로 확증했다 — web `tsconfig`의 `include`가 `**/*.js`·`**/*.cjs`만 나열하고 `**/*.mjs`가
  빠져 있어 checkJs여도 tsc가 스크립트를 잡지 않고, `next lint`는 `scripts/`가 대상 밖이다.
  admin `tsconfig`도 같고 admin `check`가 같은 패턴으로 통과 중이라는 선례까지 붙였다.
- **실 트리 돌연변이 3종**: `DB_CLIENT_OWNERS`에서 `dispatch-processing.ts`를 빼니 그 파일이
  W8 발화 · `isPublicEntry`를 항상 true로 만드니 W6 5건이 **전부 소멸** · owner prefix를 빼니
  entity api 8건이 W8 발화. **규칙과 화이트리스트가 장식이 아님**을 실트리에서 보였다.
- **사후 시뮬레이션**: 선행 정리 5건을 인메모리 적용 → W6 **0건**(다른 규칙도 0). 수정이 감시
  지점을 실제로 닫는다.
- 음성 픽스처 14/14, W8이 owner·비-owner·type-only를 정확히 구분.

**부수 관측(결함 아님)**: `src/server/db.ts`는 `export { db } from`이라 `ImportDeclaration`이
아니어서 W8이 평가하지 않는다 — owner 등록이 검출에 무해한 예비 항목이지 오류가 아니다.

**결과**: 무편집·무소득 독립 패스. 보드에 `검증: 클린 패스` 기록. 게이트② 대기.

## 이 항목의 검증 비용 — 남길 교훈

독립 패스 **4사이클**을 썼고 소득은 이랬다.

| 사이클 | 결함 | 어디에 있었나 |
| --- | --- | --- |
| 1 | 6 | 계획서 본문의 수치·줄번호 |
| 2 | 1 | 계획서 본문의 경로 접두사 |
| 3 | 1 | **내가 계획서에 중복해 쓴 라운드 기록** |
| 4 | 0 | — (기록을 걷어낸 뒤) |

**구현 지시 결함은 1사이클부터 0건이었다.** 3사이클에서 구조를 바꾸지 않았다면 4사이클도 내
기록 안의 무언가를 잡았을 것이다 — 라운드마다 기록이 늘고, 그 기록이 다음 감사의 표면이 되는
자기증식 루프였다.

교훈 둘:
1. **계획서에 라운드 기록을 중복하지 않는다.** 런북 `CLAUDE.md` 4단계가 이미 `docs/agents/main-loop/`를
   지정하는데 양쪽에 썼다. 브리핑 계약 때와 같은 부류 — 규칙이 있는데 안 따랐고 사후에 드러났다.
2. **문서 위생만 연속으로 나오면 표면을 의심한다.** 보드 정지 규칙에 이번 세션에 넣은
   "문서 위생만 2사이클 연속이면 경로 선정이 과했다는 신호"가 맞았는데, 신호를 읽고 나서도 나는
   경로가 아니라 **표면**이 문제라는 데까지 3사이클이 걸렸다.
