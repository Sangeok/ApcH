...
게이트②는 2026-08-26 사용자가 대시보드에서 개방(`15b39af`). 구현은 사용자 세션 지시로
admin-dev 디스패치(로컬) — 디스패치 전 원격 루틴 idle·오늘 #87 코멘트 0건을 확인해 동시
실행 배제. FEAT-24 행(`검토대기`)은 범위에서 명시 제외.

**인수 다섯 조건 — 전부 직접 재현:**
1. **변경 파일 ↔ 「고칠 파일」**: 코드는 `owner-banner.tsx` 1개뿐(계획과 일치) + 보드 행·백로그 제거·구현 보고.
2. **diff ↔ 「구현 스케치」**: before 블록 제거·after 오버레이 삽입이 스케치와 동일. 차이는 SVG 본문 재인덴트뿐 — 분기·조건·리터럴·노출 문구 동일. `role`/`aria-label` 제거 → `aria-hidden` 전환, `pl-[30.3%]`·`text-sm`/`text-xs`·색 hex·글꼴 계열 스케치 그대로.
3. **검증 직접 재실행**: `check` EXIT 0(lint 0 오류·tsc 통과) · `test` EXIT 0(281/60, fail 0) · `verify:fsd:final` EXIT 0 — 에이전트 보고가 아니라 메인 루프가 재실행한 출력.
4. **백로그 제거**: BUG-07 3줄 블록 소멸, 인접 FEAT-23·FEAT-24 전문 무결 직접 확인(남은 "BUG-07" 문자열 1건은 FEAT-24 source의 이력 서술 — 정상).
5. **상세 기록 실재**: `docs/agents/admin-dev/BUG-07.md`(4.2KB) — B-3 대조·스케치 대비 차이·검증 출력·못 덮는 범위 수록 확인. 보드 `결과` 143자(예산 내).

「범위 밖 의존」: 없음(계획서 그대로) — 백로그 후보 없음.
「못 덮는 범위」: `docs/release-checks.md`에 등재(폰 375/320px·데스크톱 라벨 판독성·오버레이 정렬 — 배포 후 스크린샷). FEAT-07 절 하단의 2026-08-24 관찰(원장 밖 개선 후보)이 이 항목으로 해소되는 관계도 원장에 표기.
