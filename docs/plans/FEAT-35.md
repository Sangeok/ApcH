...
- **덮는 것**
  - `format-duration.test.mjs`: `parseClockToSeconds` — `"2:47.9"→167.9`, `"2:47"→167`, 맨숫자 `"167.9"→167.9`,
    `"0:05.5"→5.5`, 공백 `"  2:47  "→167`, 무효(`"2:60"`·`"2:"`·`":30"`·`"abc"`·`""`·`"-3"`·`"2:-5"`·**`"-1:30"`·`"x:30"`·`"2:xy"`**)→null,
    ...
    `"x:30"`·`"2:xy"`는 **콜론 경로의 유한성 가드**(`!Number.isFinite(minutes)` / `!Number.isFinite(seconds)`)를 지킨다 —
    `"abc"`는 콜론이 없어 맨숫자 경로만 밟으므로 그 가드를 한 번도 시험하지 않는다. 가드가 빠지면 파서가 `null`이 아니라
    `NaN`을 돌려주고, 배선이 `if (parsed !== null)`이라 `NaN !== null`이 참이 되어 **초 state가 `NaN`으로 오염된다**.
  - `boundary-snap.test.mjs`: **no-op 회귀** — value 167.9, boundaries `[165.2, 167.893, 170.3]`, "back" → 165.2
    ...
    - **forward 선택 — 앞에 경계가 둘 이상** — `snap(165.2, [165.2, 167.893, 170.3], "forward", 0.5)` → `167.9`.
      위 forward 케이스들은 앞에 경계가 **하나뿐**이라 `Math.min(...ahead)`을 `Math.max`로 바꿔도 결과가 같다.
      앞에 단어가 둘 이상인 것은 forward 넛지의 평범한 경우인데, 그 경우를 한 번도 시험하지 않으면
      "다음 단어" 대신 "가장 먼 단어"로 건너뛰는 구현이 초록으로 통과한다(독립 패스 돌연변이 MB4 생존).
    - **forward 폴백 방향** — `snap(170.3, [165.2, 167.893, 170.3], "forward", 0.5)` → `170.8`.
      폴백 케이스가 전부 back뿐이라, forward 폴백의 부호를 뒤집은 구현(`current - fallbackStep`)이 통과한다.
      그 구현은 마지막 단어 밖에서 `+`를 눌렀을 때 **뒤로 간다** — 폴백이 없애려는 "눌렀는데 이상하게 움직임"
      바로 그 증상이다(독립 패스 돌연변이 MB9 생존).
    - **forward 폴백의 `roundTenth`는 테스트하지 않는다(등가 변이)** — back 폴백과 달리 `격자값 + 0.5`는
      부동소수점에서 오차가 나지 않는다. `0.0`~`36000.0`(10시간) 격자 360,001개를 전수로 확인한 결과
      `g + 0.5 !== roundTenth(g + 0.5)`인 값은 **0건**이다(같은 범위에서 `g - 0.5`는 존재 — 그래서 back 쪽만 시험한다).
      즉 forward 폴백의 반올림을 지워도 **도달 가능한 어떤 입력으로도 구별되지 않는다.** 케이스를 추가하지 마라 —
      죽지 않는 변이를 쫓느라 라운드가 낭비된다.
