...
- 세로 위치는 `inset-0 flex flex-col justify-center`로 오버레이 박스 전체(96 유저단위 높이에
  대응하는 렌더 높이)의 수직 중앙에 두 줄을 놓는다. 원래 텍스트는 베이스라인 기준 제목
  y=38(`owner-banner.tsx:69`)·부제 y=54(`owner-banner.tsx:79`)이고, 폰트 크기(15·12)의
  대략적 ascent/descent를 감안한 실제 시각 범위는 대략 27~57(중심 약 42, 96 대비 ≈43.8%)이다.
  50%(`justify-center`가 놓는 위치)와의 차이는 6%p 미만 — 스케일된 렌더 높이 기준 몇 픽셀
  수준이라 육안 차이가 없다.
