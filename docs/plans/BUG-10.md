...
- **덮는 것**: `shared/lib/format-date.test.mjs`로 `formatDate`·`formatDateTime`가
  **런타임 로케일·타임존과 무관하게 고정 출력**을 내는지. 타임존이 `"UTC"`로 고정
  됐으므로 출력은 결정적이다. 예: `formatDateTime("2026-07-30T22:55:46Z")` →
  `"Jul 30, 2026, 10:55 PM"`, `formatDate("2026-09-27T00:00:00Z")` → `"Sep 27, 2026"`
  같은 골든 문자열 대조. 이 테스트가 "로케일·타임존 미고정으로의 회귀"(누군가
  `timeZone`을 빼거나 `toLocaleString`으로 되돌림)를 잡는다. (골든 문자열은 CI Node의
  ICU 버전에 의존하므로, 구현 시 실제 러너 출력으로 기댓값을 맞춘다.)
