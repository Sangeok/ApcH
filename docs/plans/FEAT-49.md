...
| 파일 | 변경 |
| --- | --- |
| `src/fsd/features/caption-style/model/sample-captions.ts` | `koreanSampleCues` 순수 함수 신규(라이브 Korean 큐 텍스트를 `KR_WORDS` 순환으로 치환) + `KR_WORDS` 주석에 두 번째 소비자 명시 |
| `src/fsd/features/caption-style/ui/CaptionPreviewPlayer.tsx` | `cues` useMemo에 Korean 라이브 치환 분기 추가 + import에 `koreanSampleCues` |
...
| `src/fsd/features/caption-style/model/sample-captions.test.mjs` | `koreanSampleCues` 테스트 describe 추가 |
...
### 1. `sample-captions.ts` — `koreanSampleCues` 신규 + `KR_WORDS` 주석
...
새 함수(파일 끝에 추가, `firstSampleCueText` 아래). `CaptionCue`는 이미 `:2`에서 임포트돼 있다:

```ts
// 라이브 Korean 미리보기. 실렌더는 각 큐(영어 max_word단어 묶음)를 한국어로 번역하지만
// (apps/backend/main.py:837·:840 create_korean_subtitles_with_ffmpeg), 미리보기는 렌더 없이
...
```
```
import { firstCueText, koreanSampleCues } from "../model/sample-captions";
```
...
```
  const cues = useMemo(() => {
    ...
    // Korean 라이브 미리보기만 텍스트를 한국어 샘플로 치환한다. 설정 화면 정지 샘플
    // (sample === true)은 이미 sampleCaptionWords(Korean)=KR_WORDS로 만든 큐라 그대로 둔다.
    return language === "Korean" && props.sample !== true
      ? koreanSampleCues(built)
      : built;
  }, [
    words,
    clipStart,
...
`language`는 이미 `:40`에서 구조분해돼 있고, `displayText`(`:112`)·`activeText`·`cuesRef`는 모두 이 `cues`를 읽으므로 아래 코드는 손대지 않는다. 언어 판정은 기존 `language === "Korean"`을 그대로 쓴다 — 폰트(`:108`)·`getPreviewFontPx`의 `EM_SCALE`·`languageDefaultMaxWords`가 전부 같은 판정이라 일관된다.
...
- **FEAT-44**는 이 폴더의 `main.py:\d+` 주석을 앵커로 바꾼다. 이 항목은 그 주석을 건드리지 않는다 — 위 스케치의 새 주석은 `main.py:837`·`:583-585`를 **인용만** 하고, FEAT-44가 나중에 앵커로 교체할 대상이다(동시 진행 안 함, 게이트① 기록). `review-language-notice.ts` 주석 편집도 `main.py:\d+` 인용 줄이 아니라 별개 서술이라 FEAT-44 대상과 겹치지 않는다.

## 테스트

- **덮는 것**: `koreanSampleCues`를 `sample-captions.test.mjs`에 새 `describe`로. `import`에 `koreanSampleCues` 추가. 케이스:
  - 빈 큐 → `[]`.
  ...
  - 예상 증가: **tests 154 → 약 159(+5), suites 35 → 36(+1)**.
- **못 덮는 범위**: 
  ...
  - 라이브/샘플 분기 선택(`sample !== true`)과 안내 문구가 실제 화면에서 옳은 소비자에 뜨는지는 렌더 몫이라 러너로 못 덮는다.
