...
- **1안 (채택) — 보드 `검증:` 필드**: fetch 증가 0, 기존 필드 패턴(agent/area/status/근거/결과)의 자연스러운 연장, 검증 판정은 항목의 상태 성격이라 상태 기계(보드)에 두는 것이 의미상 맞고, 렌더는 `BudgetFlag`만큼 단순하다. 미지 필드는 현 파서가 조용히 건너뛰므로(`board.ts:22` FIELD_RE 매치 실패, 실행 확인) **표시 경로는** 규약 먼저·코드 나중이 안전하다 — 파서 배포 전에 `검증:` 줄이 있어도 무해하게 무시된다. 단 **bounce 경로는 순서가 반대다**: 청소 코드 배포 전에 필드 기록을 시작하면, 그 창에서 대시보드 반려가 현행 코드(status만 교체)로 돌아 줄이 살아남고, 배포 후 재작성 계획서에 거짓 「검증 통과」가 붙는다. 따라서 필드 기록은 구현 배포 뒤에만 시작한다(「범위 밖 의존」 2). 비용은 메인 루프의 필드 기록(범위 밖)과 bounce 청소(범위 내, 구현함)뿐이다.
