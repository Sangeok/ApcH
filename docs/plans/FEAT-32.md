...
- 번들: 기본 통합에 tracing이 포함될 수 있다 — `client/index.js:85-103` `getDefaultIntegrations()`가 `__SENTRY_TRACING__`이 undefined/truthy면 `browserTracingIntegration()`를 push한다. Replay는 기본 통합이 아니다(추가해야만 들어옴). `webpack.js:553-574` `setupTreeshakingFromConfig`는 `webpack.treeshake.removeTracing`→`__SENTRY_TRACING__=false`, `removeDebugLogging`→`__SENTRY_DEBUG__=false`를 DefinePlugin으로 심어 해당 코드를 tree-shake한다(webpack 빌드 한정 — 프로덕션 `next build`는 webpack이다).
