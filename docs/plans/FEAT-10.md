...
**결정 1 — 진행 신호원: ⓐ 이슈 #87 코멘트.** ⓐⓑⓒ를 실측(2026-08-16, `curl`)으로 비교했다:
...
**불변식 유지(제약).** 라벨·설명·pill 문구는 전부 **표시용 문자열**이고 서버로 명령 본문으로 가지 않는다. 실행 버튼은 여전히 `postPipelineCommand("pipeline-run")` — **key만** 보내고 서버가 `commands.ts` 화이트리스트에서 고정 본문을 해석한다. **라벨만 동적, 본문은 불변**(아래 스케치 §6에서 명시). DB 무접근·외부 쓰기 둘(코멘트 POST·보드 커밋 PUT) 유지 — 진행 read는 새 쓰기가 아니라 읽기다.
...
**3라운드(2026-08-17) — 새 경로 셋.** ① **실제 `next build`**(패치 트리, `next@15.5.7`) — 컴파일·린트·타입체크·정적 생성 전부 통과하고 `/pipeline`이 4.58 kB / First Load 124 kB로 나온다. 서버/클라이언트 경계도 유효하다(`"use server"` 모듈을 클라 컴포넌트가 임포트하는 사슬, 타입 전용 임포트 소거). ② **속성 기반 무작위 검증** — 시드 고정 4만 건으로 `deriveProgress`(대기상태 ↔ 미응답>0 동치, 명령 없으면 idle, `minutes ≥ 0`, `responded.sinceIso`=최신 명령, 답글 추가가 대기를 만들지 않는 단조성)와 `describePipelineRun`(`enabled` ↔ actionable 존재, 빈 문구 없음, **문구에 `undefined`/`null` 누출 없음**, 라벨이 첫 actionable id 포함)을 검사 — 위반 0건. ③ **발주 계약 대비 요구 충족 감사**(백로그 관측 3 · 요구 4 · 제약 · 범위 밖을 계획 요소에 일대일 대응). 이 세 번째가 결함을 하나 더 잡았다.

| # | 결함 | 실측 증거 | 반영 |
| --- | --- | --- | --- |
| 8 | **도장 직후 안내와 동적 라벨이 서로 모순된다** — 토스트는 "이제 실행을 누르세요"라 하는데 그 순간 버튼은 비활성이고, 설명은 방금 찍은 도장을 또 찍으라고 한다 | raw CDN 헤더 실측 `Cache-Control: max-age=300`(+`X-Cache: HIT`) → 도장 커밋 후 최대 5분간 투영이 옛 보드다. 그 보드로 `describePipelineRun` 실행: `{enabled:false, "진행할 작업 없음", "결재함 항목에 도장을 찍으면 실행할 작업이 생깁니다."}`. 백로그 관측 1도 사용자가 이 지연을 직접 겪었다고 기록한다 | 토스트를 "**보드에 반영되면** …누르세요"로, 게이트대기 설명에 "방금 찍었다면 보드 반영까지 최대 5분 걸립니다"를 더했다. 결정 5·6과 「디자인 방향」·전 경우 표·테스트 명세에 전파 |

요구 충족 감사 결과(8번 외에는 누락 없음): 관측 1 → 결정 5의 안내 카피 + 8번 수정, 관측 2 → 결정 1의 동적 라벨, 관측 3(무게중심) → 결정 1~4의 진행 pill. 요구 (a) 보드 상태 반영 라벨 → `describePipelineRun` 전 경우 표, (b) 도장 직후 안내 → `gateNextActionHint`, (c) 진행 표시 → 15초 폴링 pill, (d) **실패 가시성 → FIFO 짝짓기**(백로그가 "루틴의 1명령1답글 보장을 화면이 확인해 주지 않는다"고 지목한 바로 그 확인이다), (e) CDN 잔상 판단 → 결정 6(범위 밖 + 그 대가를 화면이 설명). 제약(화이트리스트 본문 고정·클라는 key만·DB 무접근·외부 쓰기 둘) 유지, 범위 밖(실시간 스트리밍) 준수.
