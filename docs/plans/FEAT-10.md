...
- **비활성 상태**: 진행할 작업이 없으면 버튼은 `disabled`(라벨 `"진행할 작업 없음"`), 설명이 다음 행동을 가리킨다("결재함 항목에 도장을 찍으면 …").
...
**도장 직후 안내 (카피).** 결재함 도장 성공 토스트에 다음 행동을 잇는다 — `계획지시로 넘겼습니다. 이제 파이프라인 실행을 눌러 계획서를 받으세요.` **결정(보드 커밋)과 실행(세션)이 별개**임을 말이 명시한다. FEAT-08이 기각한 자동 게시와 다르다 — 자동으로 잇지 않고 **다음 손잡이를 가리킬 뿐**, 클릭은 여전히 사용자 몫이다. 어휘 일관성: 버튼 라벨 `계획서 작성` → 도장 안내 `계획서를 받으세요` → 실행 후 pill `응답 옴`.
...
| 게이트대기(승인대기·검토대기)만, actionable 0 | false | `진행할 작업 없음` | `결재함 항목에 도장을 찍으면 실행할 작업이 생깁니다.` |
...
- 결과 예: `계획지시로 넘겼습니다. 이제 파이프라인 실행을 눌러 계획서를 받으세요.` / `구현승인으로 넘겼습니다. 이제 파이프라인 실행을 눌러 구현을 받으세요.` `gateNextActionHint`는 순수(run-plan.ts)라 client 번들에 섞여도 안전(런타임 임포트 없음).
