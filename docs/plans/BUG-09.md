...
**`route.ts`** — 앞단 가드(`:26-37`)의 **로직은 유지**하고, `:39` `return portalHandler(req);`와 위임 핸들러(`:8-24`)를 제거한 뒤 GET 본문을 아래로 바꾼다. `cancelSubscription`(`billing/api/index.ts:76-88`)과 같은 형태다.

> **아래 before는 바이트 그대로가 아니라 구조 표시다** — `:8-24`를 `/* ... */`로 접었다. 이 변경은 부분 패치가 아니라 **파일 전체 교체**이므로 after 블록이 완성된 새 `route.ts` 전문이다.
>
> **`:27-28`의 주석은 의도적으로 사라진다.** 현재 문구는 `// 포털 핸들러에 빈 문자열을 넘기면 Polar가 불투명한 오류를 돌려준다.`인데, 교체 후에는 포털 핸들러 자체가 없어 문장이 낡는다. 가드가 남는 이유는 이제 "빈 문자열 방어"가 아니라 "로그인·고객 등록 여부에 따라 갈 곳을 준다"이고, 그 절반은 `:28` 둘째 줄이 이미 말하고 있다. after 블록에 주석을 되살리지 않는 것이 맞다.
