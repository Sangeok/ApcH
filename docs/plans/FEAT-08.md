...
- `ui/pipeline-command.tsx:10-42` `PipelineCommandButton`은 `"use client"`로 `useTransition`(`:19`) + `postPipelineCommand` 호출 + 토스트(`:24-29`) + `Button`(shadcn). FEAT-07에서 선택적 `className`(prop `:13·17`, 전달 `:37`)이 붙었다.
