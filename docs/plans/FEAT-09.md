...
- **(b) 보류** — "지금은 안 한다". `승인대기`·`검토대기` → `보류`. status 줄 교체 **+ `결과:` 줄 삽입**(사유를 보드에 남긴다, 백로그 요구 2). 폐기가 아니라 대기라 백로그에는 남는다. 재개는 보드 안내 블록 규칙(`PROJECT_BOARD.md:16`, `계획지시` 또는 `구현승인`으로 되돌림).
...
- **CLAUDE.md 테스트 표(읽기 전용 — 직접 수정 금지):** `transitions.test.mjs` 행은 이미 있다(`:48`). 이 항목은 **새 파일이 아니라 기존 파일에 테스트 추가**라 파일 수(8)는 그대로고 테스트 수만 는다 — CLAUDE.md의 "8개 파일, 69개 테스트"(`:39`) 중 테스트 수 갱신과 `:48` 행 설명에 "반려 전이(되돌리기·보류·폐기) 화이트리스트·결과 줄 삽입·행 제거·다중 등장" 추가를 B단계 `비고:`로 보고한다. 또 Common Gotchas(`:129`)의 "커밋 경로의 안전은 transitions.ts 화이트리스트가 진다 — 두 전이 외에는 커밋되지 않는다"가 좁아졌으므로(반려 전이 추가) 그 문구 갱신도 함께 보고.
...
- **폐기를 status 전이(예: `폐기` status 신설)로** — 행 제거 대신 status만 바꾸면 되돌리기 쉽다. 하지만 백로그가 "보드 행 제거 + 되돌릴 수 없는 유일한 것"으로 명시했고, 신규 status 금지(백로그 「참고」)다. **채택 안 함** — 행 제거로 간다(git revert로 복구 가능하나 의도는 최종).
