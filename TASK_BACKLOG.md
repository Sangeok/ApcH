...
- [ ] **FEAT-13**: 결재함에서 게이트② 승인 전에 계획서 검증 통과 여부가 보이게
  - area: apps/admin/src/fsd (board 파서 + 결재함 카드) + PROJECT_BOARD.md 안내 블록
  - source: 소유자 승인 (2026-08-18). 관측: 결재함 카드는 `검토대기` 항목에 검증 전이든 후든 같은 승인 버튼을 보여준다(`pages/pipeline/ui/index.tsx:116`) — 원격에서는 클린 패스 여부를 알 수 없다. 검증 기록은 `docs/agents/main-loop/<항목ID>.md`에 쓰이지만(보드 안내 블록, CLAUDE.md:35) 대시보드에 렌더 경로가 없다 — `getAgentReportIndex`(`entities/agent-report/api/queries.ts:51`)는 전 폴더를 받아 오는데 `pages/pipeline/ui/_component/pixel-office.tsx:272`는 ROSTER_ORDER 책상만 `.get()`한다. 진단(추정): 검증 "판정"은 상태 성격인데 활동 기록에만 남아 화면에 닿지 않는다.
  - 대안 (계획서에서 확정한다):
    - 1안 (유력): 보드 항목에 `검증:` 한 줄 필드 — 파서 FIELD_RE 확장 + 결재함 카드 한 줄. fetch 증가 0. 미지 필드는 현 파서가 조용히 건너뛰므로 규약 먼저, 코드 나중이 가능
    - 2안: 새 status `검증완료` — 검증 전 승인을 구조적으로 막는다. 단 게이트 사용자 전권 철학(FEAT-12도 게이트 미경유 발주였다)과 어긋남
    - 3안: `docs/agents/main-loop/` 파일 목록을 결재함 행에 조인 — 존재만 전달(존재 ≠ 클린 패스), 판정까지 가려면 6번째 fetch owner 필요
  - 비고: 1안이면 반려(`검토대기`→`계획지시`) 시 옛 `검증:` 줄을 지우는 규칙이 필요하다(`transitions.ts` 최소 edit 화이트리스트 경유) — 재작성된 계획서에 옛 판정이 붙으면 이 필드가 막으려는 오판을 이 필드가 일으킨다. 보드 안내의 "검증 기록은 보드에 쌓지 않는다"는 라운드 상세(파일)와 요약 판정(필드)을 구분하도록 한 줄 수정.
