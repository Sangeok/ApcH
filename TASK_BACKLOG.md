# TASK_BACKLOG

> 최초 부트스트랩: README.md의 Known Issues / Credit System 항목을 옮겨 담음.
> PM 에이전트는 이 파일과 PROJECT_BOARD.md만 읽고 우선순위를 판단한다.
> 보드에 올라가는 것만으로는 제거하지 않는다. 담당 에이전트가 보드에 `status: 완료`로 기록한 시점에, 그 에이전트가 여기서 해당 항목을 제거한다.
> `area`는 실제 코드 경로여야 한다. PM은 코드를 읽지 않으므로 이 값을 그대로 보드에 옮긴다 — 여기가 틀리면 보드도 틀린다.

## Backend / Pipeline

- [ ] **BUG-02**: 한국어 번역 API 실패 시 영어로 조용히 폴백됨 (사용자에게 알림 없음)
  - area: apps/backend
  - source: README Known Issues

- [ ] **BUG-03**: S3 업로드 실패에 대한 에러 핸들링 부재
  - area: apps/backend
  - source: README Known Issues

- [ ] **BUG-04**: 임시 디렉토리 정리가 파이프라인 성공/실패 여부와 무관하게 항상 실행됨
  - area: apps/backend
  - source: README Known Issues

## Credit / Billing

- [ ] **FEAT-01**: Credit System 마무리 (현재 개발 중 상태)
  - area: apps/web/src/fsd/features/billing + apps/web/src/fsd/entities/user + apps/web/src/inngest
  - source: README "Currently in Development"

## Admin / Observability

- [ ] **FEAT-04**: `/pipeline` UI/UX 개편 — 결재함·팀·보고 3구역 + 캐릭터 발화 렌더링
  - area: apps/admin
  - source: 사용자 요구 (2026-08-14). 관측: FEAT-03의 첫 화면은 보드 원문(`결과` 필드 전문)을 카드에 그대로 부어 읽기 어렵고, 사용자 결정 대기 항목(FEAT-01 승인대기)이 완료 기록과 같은 위계로 맨 아래 묻혔다. 원인(확정): FEAT-03 발주문이 기능만 요구하고 디자인 기준을 한 줄도 적지 않았다 — 요구에 없는 것은 산출물에 없다. 요구: 같은 데이터(보드 투영)를 아래 구조·표현으로 재구성한다.
    - 구조 3구역: (1) **결재함** — status가 사용자 게이트(승인대기·검토대기)인 항목만 최상단, 비었으면 "결정 대기 없음" 한 줄 (2) **팀 현황** — 에이전트별 정체성 + 현재 상태 한 줄 (3) **보고 피드** — 최신순, 기본 접힘(한 줄 요약 = `결과`/`근거` 첫 문장), 펼치면 전문
    - 렌더링: 각 에이전트를 캐릭터(아바타 + 이름 + 역할)로 표현하고 모든 정보를 **역할에 맞는 발화체 한 줄**로 나른다 — pm은 결재 요청("N일째 결재 대기입니다"), dev는 계획·완료 보고, auditor는 감사 보고. 발화는 전부 보드 상태에서 **결정적 템플릿**으로 생성한다(LLM 생성·자체 상태 저장 금지 — 투영 원칙 유지). 내용 없는 대사(인사말 류) 금지. 상대 시간("N일째") 표기. 아바타는 컴포넌트 분리로 추후 일러스트 교체 가능하게(초기엔 이모지/이니셜)
    - 시각 디자인: 완성 기준은 **"데이터 덤프가 아니라 사람에게 보고받는 화면"**이다 — FEAT-03 화면(`결과` 원문이 그대로 노출, 결정 대기가 완료 기록과 같은 위계)이 명시적 반례. 타이포 위계(항목ID·제목·발화·메타가 한눈에 구분), 발화 단위 레이아웃(등간격 카드 그리드 금지 — 결재함은 크게, 보고는 촘촘하게), 여백·밀도 설계, 상태 색 체계("결정 대기 = 강조, 완료 = 침묵"), 빈 상태 화면까지 포함한다
    - 제약: 파서(`board.ts`)·명령 액션(`command-action.ts`)·인증 구조 재사용. 모바일에서 읽혀야 한다(사용자는 폰에서 본다)

## 비고

- 위 항목의 우선순위는 아직 정해지지 않음 — PM 에이전트가 매일 이 목록에서 오늘 처리할 1~2개를 골라 PROJECT_BOARD.md에 "승인대기"로 기록한다.
- 새 이슈가 생기면 이 파일 하단에 추가한다 (형식: ID, 설명, area, source).
- `source`를 쓸 때는 **관측과 진단을 분리한다.** 무엇을 봤는지(관측)와 왜 그렇다고 생각하는지(진단)를 한 문장에 섞지 않고, 진단은 추정임을 표시한다. FEAT-02가 실제로 이렇게 틀렸다 — 관측("10분 소스에 4개 요청 시 적게 생성됨")은 맞았는데 진단("유저가 무리한 개수를 골라서")이 틀렸고, 둘이 섞여 있어 담당 에이전트가 틀린 진단을 요구사항으로 받아 계획을 세 번 썼다. 반대로 BUG-06은 `파일:줄` 근거, 이미 맞는 곳, 수정 방향까지 적혀 있어 계획이 한 번에 끝났다.
