...
- [ ] **FEAT-07**: `/pipeline` 픽셀 사무실 — Gather풍 그림체 전환 + 캐릭터 고정·상태 말풍선 + 전 책상 명령
  - area: apps/admin
  - source: 사용자 요구 (2026-08-15). 관측: FEAT-06의 플랫 기하 SVG 그림체가 사용자 판정에서 기각됨("너무 별로") — 의도는 Gather풍 픽셀이었음. 시안 7회 반복(v1~v7)으로 아래가 확정됐고, **승인된 시안과 스프라이트 격자 데이터의 원본이 `docs/design/FEAT-07/`에 있다**(pixel-office-mock.html = 확정 시안, mockgen.py = 격자·팔레트 원본). 계획서는 이 시안을 계약 기준으로 삼는다.
    - **그림체**: 픽셀 스프라이트 — 문자열 격자 데이터(팔레트 문자→색 매핑)를 SVG `<rect>`로 렌더, `shape-rendering: crispEdges`. 격자가 순수 데이터라 파서·매핑을 `.test.mjs`로 덮는다
    - **캐릭터 완전 고정(정체성)**: 외형 불변 — 정체성 팔레트(pm 인디고 `#4a4080` / admin-dev 그린 `#5f8a5a` / web-dev 브라운 `#8a6b4f` / doc-auditor 퍼플 `#7a6296` / feature-scout 틸 `#4f7d78`) + 역할 소품(서류철·노트북·노트북·돋보기·나침반). **상태에 따른 옷색·포즈 변형 금지**
    - **상태 = 말풍선**: tone색 테두리·텍스트의 픽셀풍 말풍선을 머리 바로 위에(꼬리 포함). 문구는 기존 `teamState`의 `state` 재사용. **muted면 말풍선 없음**(침묵 규칙 — 말풍선의 존재 자체가 신호)
    - **책상·명패**: 책상(상판+앞면) + 명패 — 폰트 13px(모바일 10.5px) 굵게, **책상 폭 초과 허용**(가독성 우선, 사용자 지적 반영). 역할 소품은 책상 위
    - **소유자 책상 배너**: 결재함 상단 픽셀 헤더(긴 책상 + 서류 더미 + 호박 도장 + "당신의 책상" + 동적 부제 "결재 N건이 도장을 기다립니다" / 0건이면 빈 상태 문구). **결재 카드 본문은 기존 형태 유지**(혼합안 — 정보 밀도 보존)
    - **전 책상 명령 버튼**: 기존 3개(선정·감사·조사 실행)에 **dev 「작업 진행」 추가** — 화이트리스트에 `admin-work`·`web-work` 키·본문 추가("배정된 항목을 현재 status와 런북 규칙대로 처리, 게이트 전이 금지" — status가 곧 사용자의 게이트 결정이므로 안전). FEAT-06 계획 「대안」 1의 채택 전환(사용자 결정)
    - **레이아웃**: 데스크톱 = 가로 방(벽·걸레받이·체커 바닥·소품), 폰 = **2열 격자 세로 방**, 가로 스크롤 금지
  - 제약: **움직임·상시 애니메이션 금지**(사용자 명시 — 라이브 실행 중 포즈는 후속 항목으로 범위 밖). `briefing.ts`·`commands.ts` 구조 재사용(화이트리스트 확장만). 투영 원칙(보드 상태의 결정적 도출, 자체 상태 저장 금지). 스프라이트·팔레트·말풍선 규칙은 순수 데이터/함수로 테스트 가능하게
  - out of scope: 라이브 실행 존재감(명령~답글 구간 포즈 변화), 걷기·이동 연출, 일러스트 파일 교체
