...
- [ ] **FEAT-29**: 정체 감시를 15분 cron에서 처리 건별 이벤트 감시자로 전환 — Neon 유휴 compute 깨움 제거
  ...
  - source: 사용자 관측(2026-09-02 세션, Neon 비용이 1인 사용 대비 과다). 메인 루프 등재. **관측**: `monitorPipelineHealth`(`apps/web/src/inngest/functions.ts:1053`, `cron: "*/15 * * * *"`)가 24시간 내내 15분마다 `listStuckProcessingUploadedFiles`(`apps/web/src/fsd/entities/uploaded-file/api/index.ts:1312`)로 DB를 조회한다. Neon autosuspend(5분)와 겹쳐 하루 ≈96회 compute를 깨우며, 처리 중인 업로드가 없어도 돈다. DB에는 메타데이터만 있고(영상·대본은 S3 키) 저장 비용은 무시 가능하므로 비용은 compute다. **진단(코드 확정)**: `processing` 상태의 쓰기는 `startUploadedFileProcessingAttempt`(`api/index.ts:690-700`) 한 곳뿐이고(그 외 `status: "processing"` 다섯 곳은 전부 `where` 절), 호출은 `processVideo`·`analyzeVideo`의 `claim-processing-attempt` 스텝(`functions.ts:396`, `:833`)뿐이다. 따라서 정체 가능 행은 예외 없이 uploadedFileId·attempt·matchKey를 아는 Inngest 런 안에서 생기며, 전수 스캔 없이 건별 감시자로 대체할 수 있다. 단건 확인 함수 `isUploadedFileAttemptStillProcessing`(`api/index.ts:773`)과 취소 이벤트 `process-video-events/cancel`(`api/index.ts:170-175`, matchKey 동봉)이 이미 있다. Inngest `step.sleep`은 재호출 방식이라 자는 동안 compute를 쓰지 않고 route.ts `maxDuration=10`과 무관하다(무료 플랜 상한 7일). **수정 방향**: 두 처리 함수의 claim 성공 직후 `step.sendEvent`로 `processing/attempt.claimed`(uploadedFileId·attempt·matchKey) 발송 → 새 함수 `watchProcessingAttempt`가 `step.sleep(PROCESSING_STALE_POLICY.stuckAlertMs)` 후 `isUploadedFileAttemptStillProcessing`으로 단건 확인, 아직 processing이면 `reportPipelineFailure({ kind: "stuck-processing" })` + `flushReports()`. `cancelOn: [{ event: "process-video-events/cancel", match: "data.matchKey" }]`. **concurrency 설정 없음** — account·userId 슬롯을 자는 감시자가 점유하면 유저당 1건 직렬화가 막힌다. 삭제: `monitorPipelineHealth`, `listStuckProcessingUploadedFiles`, `STUCK_SCAN_LIMIT`·`truncated`, `stuckAlertMaxAgeMs`와 cron 전제 주석(`stale-policy.ts:18-22`, `functions.ts:1058-1059`); `route.ts` 등록 목록 교체; `client.ts` Events에 새 이벤트 스키마 추가. 트리거를 기존 `process-video-events`에 얹는 변형은 account concurrency 때문에 발송 시각≠claim 시각이라 재수면 분기가 필요해지므로 택하지 않는다. **커버리지 동률**: 현재 cron도 `processing`만 본다(`api/index.ts:1326`); `queued` 정체는 지금도 `reconcileStaleUploadedFileForUser`가 조회 시점에 처리. 알림은 15분마다 최대 96회 재보고(`stale-policy.ts:18`)에서 1회로 준다(Sentry fingerprint `[kind]`라 이미 한 이슈로 묶임). **범위 밖**: Neon 콘솔 설정(scale-to-zero·0.25 CU 최소·불필요 브랜치 정리)과 로컬 `.env`가 프로덕션 엔드포인트를 공유하는 문제는 코드 밖 사용자 작업.
