...
- [ ] **FEAT-17**: 행위자 상세 페이지의 역할 정의를 제목 주도 점진 공개로 — 첫 절만 펼치고 나머지 `##` 절은 접힌 details로
  ...
  - source: 소유자 관측(2026-08-23) — `/pipeline/agents/<id>`의 「역할 정의」가 정의 파일 본문 전체를 한 덩어리로 렌더해(`build-profile-view.ts:58`, `agent-profile/ui/index.tsx:49-52`) 역할 소개가 아니라 지시문 전문 열람이 된다. 요구: (1) 렌더 전에 마크다운을 **펜스 밖** `##` 경계로 분할하는 순수 함수 — `pm.md:57`의 `## YYYY-MM-DD`는 코드 펜스 안 예시라 절로 찢으면 안 된다(핵심 테스트 케이스) (2) 첫 덩어리(`# 역할` ~ 첫 `##` 전)는 펼쳐 표시, 나머지 절은 각각 native `<details>`(FEAT-04 「보고」 피드 선례)로 접고 절 제목을 summary로 (3) 절 이름 문자열에 UI를 결합하지 않는다 — 이름 기반 특별 취급 금지(정의 파일 7개의 절 제목이 제각각이라 개명 시 조용히 깨진다) (4) 출처는 정의 파일 하나 유지 — frontmatter에 화면용 필드를 추가하지 않는다
