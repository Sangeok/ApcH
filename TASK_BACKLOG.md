...
- [ ] **FEAT-10**: `/pipeline` 명령 버튼이 무엇을 실행하는지 보이게 — 라벨 명시화 + 도장 후 다음 행동 안내
  - area: apps/admin/src/pipeline + apps/admin/src/ui
  - source: 사용자 요구 (2026-08-16, FEAT-08 첫 실사용 직후). **관측 둘**:
  ...
  - **요구(방향 — 구체안은 계획에서 판단)**
    ...
    - 실행 후 **진행 중임을 알 수 있어야 한다** — 원격 세션은 1~2분 걸리는데 지금은 토스트 한 번이 전부다. 이슈 답글이 달렸는지를 화면에서 알 방법이 없다(폰 푸시 알림은 claude.ai 쪽 설정이라 대시보드 밖)
