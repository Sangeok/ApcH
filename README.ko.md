...
- 크레딧은 **생성된 클립 1개당 1개** 차감됩니다. 3클립 실행이 완료되면 체험 잔액을
  모두 사용합니다. 클립이 하나도 생성되지 않은 실행은 차감하지 않지만, **일부만
  완료된 실행(요청보다 적게 생성됐거나 에러로 끝난 경우 포함)은 생성된 클립 수만큼
  차감됩니다.**
...
- 임시 디렉토리는 프로덕션에서 성공/실패와 무관하게 정리됨(의도된 동작 — 컨테이너가 재사용됨). 로컬 `modal run` 디버그에서는 `KEEP_TEMP_ON_FAILURE=1`로 실패 시 보존 가능
