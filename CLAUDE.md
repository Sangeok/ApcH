...
| `docs/release-checks.md` | 배포 확인 원장 — 테스트가 못 덮어 배포 실물에서만 닫히는 확인 항목의 상태. 등재·마감 규칙은 파일 머리말 |
...
7. 메인 루프가 완료를 인수한다 — 보드 안내 블록의 인수 조건 **다섯**을 직접 재현(상세 기록의 실재 확인 포함). 인수한 계획서에 「범위 밖 의존」이 있으면 백로그 후보로 사용자에게 제시하고, 승인된 것은 `TASK_BACKLOG.md`에 등재한다 — 등재 안 된 후속은 pm의 입력에서 증발한다(BUG-02 웹 절반이 실측 사례)
...
9. `doc-auditor` 디스패치 → 감사 보고를 사용자에게 전달하고, **메인 루프가 `docs/agents/doc-auditor/감사기록.md`에 이어 붙인다**(그 에이전트에겐 쓰기 도구가 없다). `feature-scout`도 같다
